...
function Pagination({
    ...
    return (
    <section className='max-w-[450px] mx-auto shadow-lg rounded-lg px-5 py-3'>
        {children}
                ...
                <nav aria-label="pagination" className="isolate inline-flex -space-x-px rounded-md shadow-xs">
                    ...
                    {Array.from({ length: pagination?.totalPages }, (_, index) => (
                        <button
                            ...
                            value={index + 1}
                            onClick={handlerPagination}
                            className={ `relative inline-flex items-center ${ index + 1 === pagination?.currentPage ? currentBtn : defaultBtn }` }
                        >
                            {index + 1}
                        </button>
                    ))}
